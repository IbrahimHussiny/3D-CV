...
def ssd(feature_1, feature_2):
    ...
    return np.sum(((feature_1 - feature_2)**2))
...
def stereo_matching(img_left, img_right, K_SIZE, disp_per_pixel):
	...
	for i in range (H_BOUND[0] + padding_offset,H_BOUND[1] - padding_offset):
		for j in range(W_BOUND[0] +  padding_offset,W_BOUND[1] - padding_offset):
			...
			#max value to start comparing with
			match_diff = sys.maxint
			disparity = 0
			patch_diff = 0
			score_vector =[]
			for z in range(padding_offset,j):
				right_patch = sub_pixel_crop(img_right,i,z,K_SIZE)

				patch_diff = ssd(left_patch, right_patch)
				score_vector.append(patch_diff)
				#print pixel_diff
				if patch_diff< match_diff:
					match_diff = patch_diff
					disparity = abs(z-j)
			if not is_outlier(np.array(score_vector)):
	...
	cv.imwrite(EXPNAME+ "ssdDisparity7x7.jpg",disparties)
	write_depth_to_image(depths, EXPNAME+ "ssdDepth7x7.jpg")
	ply_creator(np.array(points3D),EXPNAME+'ssdDisparity7x7_reconstruction')
def main():
	# Set parameters
