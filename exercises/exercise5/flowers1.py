...
def stereo_matching(img_left, img_right, K_SIZE, disp_per_pixel):
	...
	for i in range (H_BOUND[0],H_BOUND[1]):
		for j in range(W_BOUND[0],W_BOUND[1]):
			left_pixel = np.mean(np.array(img_left[i][j]))
			match_diff = sys.maxint
			disparity = sys.maxint
			for z in range(j-DISPARITY_BOUND[1],j-DISPARITY_BOUND[0]):
				right_pixel = np.mean(np.array(img_right[i][z]))
				pixel_diff = linalg.norm(np.absolute(np.subtract(left_pixel,right_pixel)))
				pixel_diff = np.absolute(left_pixel-right_pixel)
				#print pixel_diff
	...
	cv.imwrite(EXP_NAME+"pixelDisparity.jpg",disparties)		
	write_depth_to_image(depths, EXP_NAME+"pixelDepth.jpg")
	#raise NotImplementedError
	return True
